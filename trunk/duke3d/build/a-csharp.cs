...
namespace build
{
    public static class A
    {
        ...
        public static void hlineasm4tran(int cnt, int skiploadincs, int paloffs, uint by, uint bx, int pbase)
        {
            //   if (skiploadincs == 0) 
            //    { 
            gbxinc = asm1;
            ...
            for (; cnt >= 0; cnt--)
            {
                ...
                byte ch = gbuf[gbufpos + ((bx >> (32 - glogx) << glogy) + (by >> (32 - glogy)))];
                if (ch < 230)
                {
                    Engine._device._screenbuffer.Pixels[pbase] = Engine._device._palette._palettebuffer[Engine.palette.palookup[(ghlinepalpos + paloffs) + ch]];
                }
                bx -= (uint)gbxinc;
                by -= (uint)gbyinc;
                pbase--;
