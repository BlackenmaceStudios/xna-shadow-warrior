...
Ported to Silverlight/XNA C# 05/04/2011 - Justin Marshall, JV Software
*/
//-------------------------------------------------------------------------
...
namespace duke3d.game
{
    public static class Gamedef
    {
        ...
        const int NUMKEYWORDS  =   112;
        public const int FOURSLEIGHT = (1<<8);

        string[] keyw = new string[NUMKEYWORDS]
        {
            "definelevelname",  // 0
            "actor",            // 1    [#]
            "addammo",   // 2    [#]
        ...
        bool isaltok(char c)
        {
            return ( Char.IsNumber(c) || c == '{' || c == '}' || c == '/' || c == '*' || c == '-' || c == '_' || c == '.');
        }
        ...
        void getglobalz(short i)
        {
            ...
            if( s.statnum == 10 || s.statnum == 6 || s.statnum == 2 || s.statnum == 1 || s.statnum == 4)
            {
                ...
                Engine.board.getzrange(s.x,s.y,s.z-(FOURSLEIGHT),s.sectnum,ref Game.hittype[i].ceilingz,ref hz,ref Game.hittype[i].floorz,ref lz,zr,Engine.CLIPMASK0);

                if( (lz&49152) == 49152 && (Engine.board.sprite[lz&(bMap.MAXSPRITES-1)].cstat&48) == 0 )
                {
                    lz &= (bMap.MAXSPRITES-1);
                    if( badguy(Engine.board.sprite[lz]) && Engine.board.sprite[lz].pal != 1)
                    {
                        if( s.statnum != 4 )
                        {
