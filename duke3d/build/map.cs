        ...
        public bool cansee(int x1, int y1, int z1, short sect1, int x2, int y2, int z2, short sect2)
        {
	        ...
	        for(dacnt=0;dacnt<danum;dacnt++)
	        {
                ...
                for (cnt = sec.wallnum; cnt > 0; cnt--, wallnum++)
		        {
                    ...
                    t = y21 * x31 - x21 * y31; if ((uint)t >= (uint)bot) continue;
                    t = y31 * x34 - x31 * y34; if ((uint)t >= (uint)bot) continue;
	        ...
	        for(i=danum-1;i>=0;i--) if (clipsectorlist[i] == sect2) return(true);
	        return false;
        }

        public bool canseefast(int x1, int y1, short sect1, int x2, int y2,  short sect2)
        {
            sectortype sec;
            walltype wal, wal2;
            int i, cnt, nexts, x, y,  cz = 0, fz = 0, dasectnum, dacnt, danum;
            int x21, y21, x31, y31, x34, y34, bot, t;
...
// jv end
        ...
        private void hline(int xr, int yp)
        {
            ...
            if (floorTrans)
            {
                A.hlineasm4tran(xr - xl, 0, s, (uint)(globalx2 * r + globalypanning), (uint)(globaly1 * r + globalxpanning), frameoffset + Engine.ylookup[yp] + xr);
            }
            else
            {
                A.hlineasm4(xr - xl, 0, s, (uint)(globalx2 * r + globalypanning), (uint)(globaly1 * r + globalxpanning), frameoffset + Engine.ylookup[yp] + xr);
            }
        }
        ...
        public bool _floorskipped = false;
        public bool floorskipped
        {
            get
            {
                return _floorskipped;
            }
        }
